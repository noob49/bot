...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7740649057:AAEt-rrqk86RO07Vl2zpbLvM5s3CYwYTH2M'
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        ...
        help_text = (
            ...
            "*🥶 /bgmi* - Trigger an bgmi operation.\n"
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        ...
        "*🗣️𝘽𝙐𝙔 :- @TRUSTVIP_MOD*", parse_mode='Markdown')
        return
...
async def remove_user(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*☠️ WITHOUT ADMIT CARD ENTRY NAHI AHI FEES JMA KRKE AAO :- @TRUSTVIP_MOD .*", parse_mode='Markdown')
        return
...
async def bgmi(update: Update, context: CallbackContext):
    ...
    # Check if the user is allowed to use the bot
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*☠️ WITHOUT ADMIT CARD ENTRY NAHI AHI FEES JMA KRKE AAO :- @TRUSTVIP_MOD *", parse_mode='Markdown')
        return
...
async def run_bgmi(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"*⚠️ Error during the bgmi: {str(e)}*", parse_mode='Markdown')

    finally:
        await context.bot.send_message(chat_id=chat_id, text="*✅ bgmi Completed! ✅*\n*Thank you for using our service!*", parse_mode='Markdown')
